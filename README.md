...
A Kotlin compiler plugin that allows `@HidesMembers` to work for more declarations than just `forEach`
and `addSuppressed`.

In short, this plugin allows using `@HidesMembers` so that you can declare an extension function that takes over a
member function in overload resolution.
